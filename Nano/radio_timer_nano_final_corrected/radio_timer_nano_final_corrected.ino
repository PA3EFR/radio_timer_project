/*
 * Radio Zendtijd Timer - Arduino Nano (FINALE VERSIE MET CORRECTE BUZZER LOGICA)
 * VERSIE MET BUZZER OP A3 DIE ALLEEN SPEELT TIJDENS TOON EN BEEP FASEN
 * 
...
#define TONE_PIN          4      // 1kHz toon output (tone functie)
#define BEEP_PIN          5      // Beep piepjes output (tone functie)
#define BUZZER_PIN        A3     // Buzzer output (tone functie) - verplaatst van D6
...
#define BUZZER_FREQUENCY   800      // 800Hz buzzer (betere hoorbaarheid)
...
bool buzzerActive = false;           // Nieuwe buzzer state tracking
unsigned long timerStartTime = 0;
unsigned long lastBeepTime = 0;
unsigned long warningToneStartTime = 0;
...
unsigned long warningTimeMs = 0;
unsigned long beepTimeMs = 0;
...
void setup() {
  ...
  pinMode(BUZZER_PIN, OUTPUT);       // Nieuwe buzzer output op A3
  pinMode(PTT_RELAY_PIN, OUTPUT);
  pinMode(LED_STATUS_PIN, OUTPUT);
  
  // Analoge pinnen configureren voor Arduino Nano
...
void loop() {
  ...
  if (timerActive) {
    ...
    // ABSOLUTE TIMING: Waarschuwings tijd bereikt (Timer - 15 sec) - activeer EENMALIGE 1sec waarschuwings toon + buzzer
    ...
    // EENMALIGE TONE: Stop toon na 1 seconde + ook buzzer uit (geen doorloop)
  ...
  if (warningBeepsActive && timerActive) {
    handleWarningBeeps();  // handleWarningBeeps regelt zelf de buzzer activatie
  }
  
  // 1-second warning tone wordt nu direct gestart wanneer warningToneActive becomes true
...
void handleWarningBeeps() {
  ...
  if (currentTime - lastBeepTime >= 200) {  // Snellere interval voor pulserende beeps
    // Stop eerst toon als deze nog aanstaat
    if (warningToneCompleted) {
      noTone(TONE_PIN);
    }
    ...
    noTone(BUZZER_PIN);  // Buzzer ook uit tussen beeps
    buzzerActive = false; // Reset buzzer state
...
void generateWarningTone() {
  // Deze functie is nu leeggemaakt omdat de warning tone logica direct in loop() staat
  // voor betere controle over eenmalige activatie
}
